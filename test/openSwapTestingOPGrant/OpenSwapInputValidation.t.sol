...
contract OpenSwapInputValidationTest is Test {
    ...
    function setUp() public {
        ...
        grantFaucet = new BountyAndPriceRequest(address(oracle), address(bountyContract), faucetOwner);
    ...
    function _validOracleParams() internal pure returns (openSwapOPGrant.OracleParams memory) {
        return openSwapOPGrant.OracleParams({
    ...
    function _validSlippageParams() internal pure returns (openSwapOPGrant.SlippageParams memory) {
        return openSwapOPGrant.SlippageParams({priceTolerated: 5e14, toleranceRange: 1e7 - 1});
    }

    function _validFulfillFeeParams() internal pure returns (openSwapOPGrant.FulfillFeeParams memory) {
        return openSwapOPGrant.FulfillFeeParams({
    ...
    function _validBountyParams() internal pure returns (openSwapOPGrant.BountyParams memory) {
        return openSwapOPGrant.BountyParams({
    ...
    function testSwap_SellTokenEqualsBuyToken_Reverts() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(openSwapOPGrant.InvalidInput.selector, "sellToken = buyToken"));
    ...
    function testSwap_ZeroSellAmt_Reverts() public {
        vm.startPrank(swapper);
        vm.expectRevert(abi.encodeWithSelector(openSwapOPGrant.InvalidInput.selector, "zero amounts"));
        swapContract.swap{value: GAS_COMPENSATION + BOUNTY_AMOUNT + SETTLER_REWARD + 1}(
            0, // zero sellAmt
            address(sellToken),
            MIN_OUT,
    ...
    function testSwap_MaxFeeTooHigh_Reverts() public {
        openSwapOPGrant.FulfillFeeParams memory badFeeParams = openSwapOPGrant.FulfillFeeParams({
            startFulfillFeeIncrease: 0,
        ...
        vm.startPrank(swapper);
        vm.expectRevert(abi.encodeWithSelector(openSwapOPGrant.InvalidInput.selector, "fulfillmentFee"));
        swapContract.swap{value: GAS_COMPENSATION + BOUNTY_AMOUNT + SETTLER_REWARD + 1}(
            SELL_AMT,
            address(sellToken),
            MIN_OUT,
    ...
    function testSwap_MaxFeeBelowBoundary_Succeeds() public {
        openSwapOPGrant.FulfillFeeParams memory goodFeeParams = openSwapOPGrant.FulfillFeeParams({
            startFulfillFeeIncrease: 0,
    ...
    function testSwap_SettlerRewardTooLow_Reverts() public {
        openSwapOPGrant.OracleParams memory params = _validOracleParams();
        params.settlerReward = 99; // < 100

        vm.startPrank(swapper);
        vm.expectRevert(abi.encodeWithSelector(openSwapOPGrant.InvalidInput.selector, "oracleParams"));
    ...
    function testMatchSwap_ParamHashMismatch_Reverts() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(openSwapOPGrant.InvalidInput.selector, "params"));
    ...
    function testMatchSwap_Expired_Reverts() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(openSwapOPGrant.InvalidInput.selector, "expired"));
    ...
    function testMatchSwap_AlreadyMatched_Reverts() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(openSwapOPGrant.InvalidInput.selector, "swap matched"));
    ...
    function testMatchSwap_Cancelled_Reverts() public {
        ...
        vm.startPrank(matcher);
        bytes32 swapHash = swapContract.getSwapHash(swapId);
        vm.expectRevert(abi.encodeWithSelector(openSwapOPGrant.InvalidInput.selector, "swap cancelled"));
        swapContract.matchSwap(swapId, swapHash);
        vm.stopPrank();
    }
    ...
    function testMatchSwap_NotActive_Reverts() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(openSwapOPGrant.InvalidInput.selector, "swap not active"));
