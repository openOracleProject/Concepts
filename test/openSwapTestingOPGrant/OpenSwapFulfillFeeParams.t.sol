...
contract OpenSwapFulfillFeeParamsTest is Test {
    ...
    function _getOracleParams() internal pure returns (openSwapOPGrant.OracleParams memory) {
    ...
    function _getSlippageParams() internal pure returns (openSwapOPGrant.SlippageParams memory) {
        return openSwapOPGrant.SlippageParams({
            priceTolerated: 5e14,
            toleranceRange: 1e7 - 1
        });
    }

    function _getBountyParams() internal pure returns (openSwapOPGrant.BountyParams memory) {
    ...
    function _createSwapWithFeeParams(openSwapOPGrant.FulfillFeeParams memory feeParams) internal returns (uint256 swapId) {
    ...
    function testFulfillFeeParams_MaxFeeZero_Reverts() public {
        openSwapOPGrant.FulfillFeeParams memory badParams = openSwapOPGrant.FulfillFeeParams({
        ...
        vm.startPrank(swapper);
        vm.expectRevert(abi.encodeWithSelector(openSwapOPGrant.InvalidInput.selector, "fulfillFeeParams"));
    ...
    function testFulfillFee_ImmediateMatch_UsesStartingFee() public {
        openSwapOPGrant.FulfillFeeParams memory feeParams = openSwapOPGrant.FulfillFeeParams({
            startFulfillFeeIncrease: 0,
            maxFee: 100000,
        ...
        openSwapOPGrant.Swap memory s = swapContract.getSwap(swapId);
    ...
    function testGetFulfillmentFeeParams_ReturnsCorrectParams() public {
        ...
        openSwapOPGrant.FulfillFeeParams memory retrieved = swapContract.getFulfillmentFeeParams(swapId);
    ...
    function testSingleFee_EmittedWhenMaxEqualsStartAndOneRound() public {
        ...
        emit openSwapOPGrant.SingleFee(1, 10000); // swapId = 1 (nextSwapId starts at 1)
    ...
    function testFulfillFee_HigherFeeReducesFulfillAmount() public {
        ...
        openSwapOPGrant.FulfillFeeParams memory lowFeeParams = openSwapOPGrant.FulfillFeeParams({
            startFulfillFeeIncrease: 0,
            maxFee: 10000, // 0.1%
            startingFee: 10000,
            roundLength: 60,
        ...
        openSwapOPGrant.FulfillFeeParams memory highFeeParams = openSwapOPGrant.FulfillFeeParams({
            startFulfillFeeIncrease: 0,
            maxFee: 100000, // 1%
            startingFee: 100000,
            roundLength: 60,
        ...
        openSwapOPGrant.Swap memory s1 = swapContract.getSwap(swapId1);
        openSwapOPGrant.Swap memory s2 = swapContract.getSwap(swapId2);
    ...
    function testFulfillFee_LockedAtMatchTime() public {
        ...
        openSwapOPGrant.Swap memory sAfter = swapContract.getSwap(swapId);
